...
interface SimpleSoundContextType {
  isPlaying: boolean;
...
const SimpleSoundContext = createContext<SimpleSoundContextType | null>(null);

export const useSimpleSoundContext = () => {
  const context = useContext(SimpleSoundContext);
  if (!context) {
    throw new Error('useSimpleSoundContext must be used within a SimpleSoundProvider');
  }
  return context;
};
...
class LayeredAudioManager {
  ...
  async playLayeredSound(baseSound: string, layerSound?: string) {
    ...
    if (!this.baseAudio || this.baseAudio.src !== new URL(baseSound, window.location.origin).href) {
        ...
        this.baseAudio.addEventListener('timeupdate', () => {
          if (this.baseAudio) {
            ...
            if (duration - currentTime <= fadeTime && duration - currentTime > 0) {
              ...
              const targetVolume = 0.6 * fadeProgress;
            ...
            // Fade in at beginning
              ...
              this.baseAudio.volume = 0.6 * fadeProgress;
            }
            // Normal volume in middle
            else {
              this.baseAudio.volume = 0.6;
            }
          }
        });
      }
      ...
      if (baseSound.includes('outsiderain')) {
        targetBaseVolume = 1.0; // Outside rain for home page at 100% (max browser allows)
      } else if (baseSound.includes('cyberpunk')) {
        targetBaseVolume = 0.6; // Cyberpunk at 60%
      }
  ...
  togglePlayback() {
    ...
    if (this.isGloballyPlaying) {
      ...
      if (this.baseAudio) {
        let targetBaseVolume = 0.4;
        if (this.baseAudio.src.includes('outsiderain')) {
          targetBaseVolume = 1.0; // Max volume for rain
      ...
      if (this.layerAudio) {
        let targetLayerVolume = 0.3;
        if (this.layerAudio.src.includes('restaurant')) {
          targetLayerVolume = 0.3;
        } else if (this.layerAudio.src.includes('cooking')) {
          targetLayerVolume = 0.5;
        } else if (this.layerAudio.src.includes('insiderain')) {
          targetLayerVolume = 0.7;
        }
...
export const SimpleSoundProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [isPlaying, setIsPlaying] = useState(false); // Start paused
  const [currentBaseSound, setCurrentBaseSound] = useState('/sounds/outsiderain.wav');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      audioManagerRef.current = new LayeredAudioManager();
              ...
              // Prepare audio but don't start playing - user must click play button
        const prepareAudio = () => {
          setHasUserInteracted(true);
          // Load the default landing page sound but keep it paused
          if (audioManagerRef.current) {
            audioManagerRef.current.playLayeredSound('/sounds/outsiderain.wav');
          }
        };
      
      // Prepare audio on page load but keep paused
      prepareAudio();
      
      // Also add listeners as fallback for browsers that need user interaction
      const enableAudio = () => {
        setHasUserInteracted(true);
